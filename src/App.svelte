...
<div class="fixed inset-0 bg-solid-100 over">
  ...
  {:else}
    <div
      class="sm:grid sm:grid-cols-2 sm:grid-rows-1 relative"
      style="height: calc(100% - 5rem - 2px)"
    >
      ...
      {#if selectedEnvelope}
        ...
        >
          <Envelope
            id={selectedEnvelope}
            onBack={() => (selectedEnvelope = null)}
          />
        </div>
      {:else if windowWidth >= 640}
        <div class="flex justify-center items-center opacity-50 italic">
          Select an envelope to view it in detail
        </div>
      {/if}
    </div>
    ...
    <div class="h-20 border-t-2 border-base-200 flex items-center">
      ...
      <div class="badge badge-lg badge-primary m-2">
        {formatMoney(getUserTotalBalance($db))}
      </div>

      <div
        class="p-2 select-none cursor-pointer hover:opacity-80 transition-opacity"
        on:click={() => (editJson = true)}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-7 w-7"
      ...
      <div
        class="p-2 select-none cursor-pointer hover:opacity-80 transition-opacity"
        on:click={() => (settingsModalIsOpen = true)}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-7 w-7"
  ...
  <SettingsModal
    bind:isOpen={settingsModalIsOpen}
    {onLogout}
    showJSON={() => (editJson = true)}
  />
